...
if (!isset($_POST['username']) || !isset($_POST['password']) || !isset($_POST['fullname']) || !isset($_POST['email'])) {
    header('Location: ../app/register.php?error=missing_inputs');
    exit;
}
...
if ($execQuery->num_rows > 0) {
    header('Location: ../app/register.php?error=user_exists');
    exit;
}
...
if (!empty($photo['tmp_name']) && $photo != null) {
    ...
    if (!move_uploaded_file($_FILES['photo']['tmp_name'], 'uploads/' . $photo)) {
        header('Location: ../app/register.php?error=photo_upload_failed');
        exit; 
    }
}else {
...
$sql = "INSERT INTO users(fullname, email, phone, username, password, photo) VALUES('$fullname', '$email', '$phone','$username','$password','$photo')";
$execQuery = $conn->query($sql);
if ($execQuery) {
    header('Location: ../app/login.php?success=registration_successful');
    exit;
}